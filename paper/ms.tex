...
% \title{Seeing Double: Still Against the Planetary Interpretation of PTFO$\,$8-8695b}
...
\begin{abstract}
  ...
  system is a sinusoid with a ``long'' period $P_{\rm
  \ell}=11.96\,$hr, previously thought to be a systematic, but in fact likely caused by stellar rotation.  Also present
  is a complex signal, previously identified as the planet candidate,
  that repeats with a ``short'' period $P_{\rm s}= 10.74\,$hr.
  The two signals beat every 4.48 days.
  ...
  binary M dwarf in which one star shows the ``long''
  rotation signal, and the other star is showing ``transient dipping''
  that has also been observed in other young M dwarfs.
  The planetary interpretation seems quite unlikely.
\end{abstract}
