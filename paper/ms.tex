...
\begin{abstract}
  ...
  planetary transits.  Although strong evidence against the planet
  hypothesis has been presented, the possibility remains debated in
  the literature.  To obtain further clues, we inspected data from the
  NASA {\it Transiting Exoplanet Survey Satellite} (TESS) and the ESA
  Gaia mission.  The Gaia data suggest that PTFO$\,$8-8695 is a
  binary: the photometric data show it to be overluminous with respect
  to members of its kinematic group, and the astrometric data are
  inconsistent with a single star.  The TESS light curve shows two
  different photometric periods. The variability is dominated by a
  sinusoidal signal with a period of 11.98$\,$hr, presumably caused by
  stellar rotation.  Also present is a 10.76$\,$hr signal consisting
  of a not-quite sinusoid interrupted by hour-long dips, the type of
  signal previously interpreted as planetary transits.  The phase of
  the dips is nearly 180$^\circ$ away from the phase of the originally
  reported dips. As noted previously, this makes them difficult to
  explain as planetary transits.  Instead, we believe that
  PTFO$\,$8-8695 is a pair of young and rapidly rotating M dwarfs, one
  of which shows the same ``transient-dipper'' behavior that has been
  seen in at least 5 other cases.  The origin of these transient dips
  is still unknown but likely involves circumstellar material.
\end{abstract}
...
We wish \ptfob\ were a planet. It would be quite exceptional. It would be the
youngest known \added{transiting\footnote{Younger hot Jupiters have been
reported around V830 Tau, TAP 26, and CI Tau
\citep{johns-krull_CI_Tau_candidate_2016,donati_hot_2016,donati_hot_2017,yu_hot_2017,flagg_co_2019}.}}
hot Jupiter \citep{van_eyken_ptf_2012}, orbiting a T Tauri star in the
Orion-OB1a cluster.  It would have the shortest orbital period of any hot
Jupiter.  With such a short period, it would probably be filling its Roche
lobe, and actively losing mass to its host star.  Not only that, but the
rapidly-rotating host star is probably oblate enough to \replaced{torque the
planet's orbit into and out of the transiting configuration}{cause the planet's
orbit to precess into and out of the transiting configuration} on a timescale
of years
...
\replaced{the planet itself}{a close-in planet}
...
composition \replaced{are}{could be} quite short
...
variable stars is that a \replaced{circumstellar cloud of gas is
orbiting near the co-rotation radius}{clumpy torus of dust and gas
orbits near the Keplerian co-rotation radius}.  To this point, though,
...
\replaced{These details concerning the group membership for one object
may seem excessive to those accustomed to the simple distinction
between ``young cluster members'' and ``old field stars''.  Although
all members of the Orion complex are indeed young relative to the
field,} {While all members of the Orion complex are young relative to
the field,} these \replaced{details}{considerations} are essential for
assessing \deleted{the} photometric evidence for the binarity of
...
\begin{figure*}[t]
	\begin{center}
		\leavevmode
		\includegraphics[width=.93\textwidth]{f3.pdf}
	\end{center}
	\vspace{-0.7cm}
...
The tallest peak occurs at \replaced{0.499\,d
(11.98$\,$hr)}{11.98$\,$hr} and a second strong peak occurs at
\replaced{0.448\,d (10.76$\,$hr)}{10.76$\,$hr}. We will refer to these
...
\explain{NOTE: All paragraphs in this section have been heavily reworded to
emphasize RUWE over $\chi_{\rm red}^2$, as requested by the referee.}
...
goodness-of-fit\added{, many M dwarfs do, because of a color-dependent
term in the mission's PSF model} \citep[see][Appendix
...
not reduced to velocities\added{.} Further radial-velocity observations could
...
peak in the periodogram at \replaced{$0.9985 \pm
0.0061\,$days}{$23.96\pm0.15\,$hr}. They ascribed it to their
...
\replaced{$0.50\,{\rm d}$}{11.98$\,$hr} signal was indeed from stellar
...
\explain{NOTE: Citations to Parks et al 2014 and Stauffer et al 2015
were added to the paragraph below} When physical explanations are not
...
\explain{NOTE: added citation to Bodman et al 2017 in this paragraph.}
...
\begin{enumerate}[topsep=0.5ex,itemsep=-0.5ex,partopsep=1ex,parsep=1.5ex]
  \item They are weak-lined T Tauri stars\added{ (under the
    definition of {\it e.g.}, \citealt{fang_star_2009})}.
  ...
  \item The age is typically $\lesssim$ 100$\,$Myr.\footnote{ At
    ...
    meet the ``persistent dip'' classification.  If so, it is \deleted{also} the
    oldest known. } 
  \item The light curves show shallow, angular dips, \deleted{usually}
    superposed on large-amplitude smooth variability \added{at or near
    1:1 synchronicity}. The \replaced{latter}{smooth variability} is
  ...
  \replaced{of $\lesssim 1\%$}{between 0.1--1\%}
   of young M2-M5 stars \citep{rebull_usco_2018}.
\end{enumerate}
The 10.76$\,$hr signal in \ptfo\ meets all of these criteria.
\replaced{This is the first connection of \ptfo\ with this class of
objects, likely because the TESS data enabled us to put the dips in
the context of the asymmetric out-of-dip modulation.} {While this was
mentioned by \citet{stauffer_orbiting_2017}, the
asymmetric out-of-dip modulation shown by the TESS data solidifies the
connection.}
...
\replaced{and so are apparently binaries}{a possible sign of binarity}
\citep[][Table~1]{stauffer_orbiting_2017}.  This is higher than the
main-sequence companion fraction of ${\rm
CF}_{0.1\text{-}0.5\,M_\odot}^{\rm MS} = 33 \pm 5\%$\added{, which
could be an indication that the transient flux dip phenomenon is
linked to stellar binarity}
\citep{henry_solar_2006,duchene_stellar_2013,winters_solar_2019}.
\added{This fraction might be even more significant given the
selection bias against high mass ratio binaries, in which the second
star would be too faint to detect a second period.}

\added{However, the association between transient flux dips and
binarity is not iron-clad. Firstly, multiple photometric periods do
not necessarily imply binarity. For instance, circumstellar material
could exist around single stars at multiple orbital distances, and not
just at the co-rotation radius.  This could cause eclipses with
periods different from the stellar rotation period.  Additionally, }
low-mass pre-main-sequence stars \deleted{however} have been shown to
\added{have} companion fractions up to twice as high in dispersed
clusters such as Upper~Sco and Taurus
...
the primordial \deleted{gas} disks of both stars in \ptfo\ seem to be
have been depleted \added{of hot dust within the inner $\sim$1$\,$AU}.
This is consistent with the $8.5\pm1.2\,{\rm Myr}$ age of the
25$\,$Ori-1 group, and the rapid rate at which stars \deleted{show}
...
\explain{NOTE: Removed ``high-latitude accretion hotspots'' from the
...
{\it (iv)} high-latitude starspots;
or {\it (v)} dust clouds of \replaced{plausible
composition}{particular compositions
\citep[][Figure~10]{zhan_complex_2019}}.  We also view the possibility
of {\it (vi)} tidally disrupted planetary or cometary material to be
implausible, given the synchronicity between dip and rotation periods
seen across many systems.  \added{Finally, {\it (vii)} transits of an
enshrouded protoplanet can be added to the list of disfavored
explanations, based on the chromaticity, asymmetric phase curve, and
light-curve changes seen in \ptfo.}

\explain{NOTE: ``High-latitude accretion hotspots'' was moved to this
paragraph.}
The explanations that we view as not yet ruled out include {\it A)}
transiting clumps of \added{dust or }gas at the Keplerian co-rotation
radius; {\it B)} occultations of \replaced{starspots}{hotspots on the
stellar surface} by an optically thick disk \citep{zhan_complex_2019};
and {\it C)} high-latitude accretion hotspots passing behind the star
\citep{yu_tests_2015}.  \replaced{The evolution of \ptfo\
over}{Observations of \ptfo\ over} the past decade may offer some
hints.  \deleted{Specifically, }\ptfo's transition between having
no dips, one dip, and multiple dips per cycle seems important
\added{(Figure~\ref{fig:o_minus_c})}.  \deleted{The dips would then
need to be caused by material that was somehow disrupted from the
planet, but somehow remained co-orbital for an extended duration. This
seems implausible.} \added{ The H$\alpha$ excess observed by
\citet{johnskrull_h_2016} also seems like an important piece of
evidence.  }

\explain{NOTE: This paragraph is newly added, to support our view of the
high-latitude accretion hotspot being plausible for \ptfo.}
The high-latitude accretion hotspot scenario seems capable of
explaining most of the qualitative characteristics of \ptfo.  Some
degree of accretion onto the star seems likely, given 
the high-velocity ($\pm\,200$\,km$\,$s$^{-1}$) components of
the observed H$\alpha$ line profile
\citep{johnskrull_h_2016},
as well as the H$\alpha$
equivalent width of 11.4$\,$\AA\ \citep{van_eyken_ptf_2012}, which is
near the boundary separating ``classical'' and ''weak-lined'' T Tauri
stars \citep{briceno_cidaII_2019}.  The non-detection of any NIR excess could
be problematic, but does not eliminate the possibility that a tenuous
amount of dust and gas remains near the star, and is able to accrete
onto it.  The main problem with this explanation is that it seems to
require rare circumstances, making it unlikely to be correct for the
entire population of transient flux dips.  RIK-210 in particular has
dips that seem too deep ($\sim$20\%) and too asymmetric to be
explained by an accretion hot spot \citep{david_transient_2017}.

\explain{NOTE: This paragraph has been expanded to clarify the
possibility of a hotspot being occulted by a disk.}
Explanations {\it A} and {\it B} (circumstellar material at the
co-rotation radius, and occultations of spots by a disk) have the
appeal that they are flexible enough to explain not only the transient
and persistent-dip stars, but also the ``scallop shells''
\citep{stauffer_orbiting_2017}.  \replaced{Despite this appeal, the
possibility of distinct mechanisms explaining these distinct
variability classes remains open.}{ In the spot occultation scenario
\citep{zhan_complex_2019}, the ``blobs'' that provide the necessary
brightness contrast are on the surface of the star, rather than at the
co-rotation radius. The observing geometry would require an inner ring
to be projected into a narrow band in front of the stellar disk, in
order to produce variations with sufficiently short timescales.  The
outer disk would also need to either not be present, or to be
misaligned enough and insufficiently flared, that it did not obstruct
the line of sight to the star.  This geometric requirement seems too
contrived to occur in even $\lesssim$1\% of young M dwarf systems. }

\added{Thus, while several of the proferred explanations deserve
further scrutiny, we consider the clumpy torus of optically thick dust
or gas near the co-rotation radius to be most plausible.  The
partially ionized circumstellar hydrogen would readily explain the
excess H$\alpha$ emission, and changes in the clumping geometry would
lead to changes in the observed eclipse shapes.  The long-term
stability, punctuated by changes in dip phases, might be understood as
an accretion disk subject to strong magnetic fields going through
``stalled'' states in which material is trapped near co-rotation
\citep{dangelo_accretion_2012}.  Physical models of this scenario may
need to consider such states, in addition to dissociation of the dust
population
\citep{millan-gabet_circumstellar_2007,dullemond_inner_2010}.
Finally, the question of whether the opacity in fact comes from dust
or partially-ionized gas ({\it e.g.}, via gas continuum, atomic lines,
or perhaps TiO molecular opacity) remains an additional issue for
future exploration.  }
...
\explain{NOTE: Updated this paragraph to remove spot occultations by a disk as
a ``preferred explanation''.}
The physical mechanism that explains the transient and persistent flux
dips is unresolved. Our preferred explanation is that a clumpy torus
of dust or gas is near the Keplerian co-rotation radius, and
periodically occults the star.  The \added{chromaticity,
out-of-eclipse variability, and} jumping orbital phase
\replaced{disfavored}{of \ptfo\ all disfavor} the explanation of an
enshrouded, transiting protoplanet.  Though \ptfo\ may not be a
planet,\deleted{ as we and others had hoped,} understanding it and its
analogs is a worthy problem\replaced{.}{ that seems likely to provide
insight into how gas and dust disperse from young protostellar disks.}
\replaced{It might even}{Understanding this process would, in turn,}
teach us about the birth environments of the majority of
habitable-zone Earth-sized planets \citep{dressing_occurrence_2013}.
...
analysis of the Gaia data provides independent support for the
