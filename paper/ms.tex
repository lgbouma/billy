...
\citep{donati_hot_2016,donati_hot_2017,yu_hot_2017}.}} hot Jupiter \citep{van_eyken_ptf_2012}, orbiting
...
probably oblate enough to \replaced{torque the planet's orbit into and out of
the transiting configuration}{cause the planet's orbit to precess into and out of the 
...
\replaced{the planet itself}{a close-in planet} \citep{johnskrull_h_2016}.  In addition to the
...
composition \replaced{are}{could be} quite short \citep{zhan_complex_2019}. 
...
variable stars is that a \replaced{circumstellar cloud of gas is orbiting near
the co-rotation radius}{clumpy torus of dust and gas orbits near the
Keplerian co-rotation radius}.  To this point, though, it has not been clear
...
\replaced{These details concerning the group membership for one object may seem
...
{While all members of the Orion complex are young relative to the field,}
these \replaced{details}{considerations} are essential for assessing \deleted{the} photometric evidence for
...
The tallest peak occurs at \replaced{0.499\,d (11.98$\,$hr)}{11.98$\,$hr} and a second strong
peak occurs at \replaced{0.448\,d (10.76$\,$hr)}{10.76$\,$hr}. We will refer to these two
...
\explain{All paragraphs in this section have been heavily
	reworded to emphasize RUWE over $\chi_{\rm red}^2$, as requested by the
	referee.}
...
degrees of freedom. The reduced $\chi^2$ is 2.80.  While the majority of
...
term in the mission's PSF model} \citep[see][Appendix A]{lindegren_gaiasoln_2018}.
The reduced $\chi^2$ has been
``renormalized'' to account for this, yielding a modified
statistic called the renormalized unit weight error
...
We acquired the RUWE for each of the
149 members in 25$\,$Ori-1 identified by
identified by \citet{kounkel_apogee2_2018}.
We then queried the
...
The lower panel of Figure~\ref{fig:gaia} shows the RUWE of
these stars as
a function of stellar brightness.  \ptfo\ is at the 93$^{\rm
	rd}$ percentile of equally-variable stars within the
25$\,$Ori-1 group.  
Two of the thirty stars
...
magnitudes, and shows both a strong Wide-field Infrared Survey
Exoplorer (WISE, \citealt{wright_WISE_2010}) IR excess and also a
10$\mu$m silicate emission feature \citep{mauco_herschel_2018}.  The
other star was GAIA DR2 3222210363837122048.

Potential explanations for an elevated RUWE include
photometric variability and unresolved stellar binarity \citep[{\it
e.g.},][]{rizzuto_ZEIT8_2018,belokurov_unresolved_2020}.  
If photometric variability were the root cause, we would expect stars with
similar brightness and color in the same kinematic group of Orion to show
similar astrometric excesses, because the majority of young stars are
highly variable.
However
relative to other M-dwarf group members with
...
peak in the periodogram at \replaced{$0.9985 \pm 0.0061\,$days}{$23.96\pm0.15\,$hr}. They ascribed it
...
\replaced{$0.50\,{\rm d}$}{11.98$\,$hr} signal was indeed from stellar rotation.  The TESS
...
\explain{Note: Citations to Parks et al 2014 and Stauffer et al 2015 were
	added to the paragraph below}
When physical explanations are not forthcoming, we often resort to
taxonomy.  
By searching the literature, we found about a dozen light curves with
similar morphologies to \ptfo, drawn from surveys of low-mass
weak-lined T Tauri stars in regions including $\rho$ Oph, Upper~Sco,
Taurus, and perhaps the Pleiades
...
\citep{ansdell_young_2016,bodman_dippers_2017,cody_manyfaceted_2018}.  The persistent and
...
\begin{enumerate}[topsep=0.5ex,itemsep=-0.5ex,partopsep=1ex,parsep=1.5ex]
  ...
  \item \replaced{The age is typically $\lesssim$ 100$\,$Myr.}{The
    ages typically range between 5 and 100$\,$Myr.}\footnote{ At
    ...
    oldest known. \added{The lower limit comes from the lack of detectable
...
\added{This fraction might be even more significant given the
	selection bias against high
mass ratio binaries, in which
the second star would be too faint to detect a second period.}

\added{However, the association between transient flux dips and binarity is
not iron-clad.  Firstly, multiple photometric periods do not necessarily
imply binarity. For instance, circumstellar material could exist
around single stars inside, at, or wide of co-rotation.  Additionally, } low-mass pre-main-sequence stars
\deleted{however} have been shown to \added{have} companion fractions
up to twice as high in dispersed clusters such as Upper~Sco and Taurus
...
the primordial \deleted{gas} disks of both stars in \ptfo\ seem to be have been
depleted \added{of hot dust within the inner $\sim$1$\,$AU}.  This is consistent with the $8.5\pm1.2\,{\rm Myr}$ age of
the 25$\,$Ori-1 group, and the rapid rate at which stars \deleted{show} lose
their disks between 1 and 10$\,$Myr \citep[{\it
...
The key question is what causes the transient dips. This is
...
or
{\it (vi)} dust clouds of \replaced{plausible composition}{particular compositions \citep[][Figure~10]{zhan_complex_2019}}.
...
The explanations that \replaced{are not yet}{have not yet been} ruled out include {\it (i)}
transiting clumps of \added{dust or }gas at the Keplerian co-rotation radius; {\it
(ii)} transits of enshrouded protoplanets; {\it (iii)} occultations of
\replaced{starspots}{hot and cold spots on the stellar surface} by an optically thick disk.  The first and last explanations
...
shell'' M-dwarfs \citep{stauffer_orbiting_2017}.  \replaced{Despite this appeal,
the possibility of distinct mechanisms explaining these distinct
variability classes remains open.}{
The model of hot and coldspot occultations by an optically thick disk
\citep{zhan_complex_2019} is a qualitative inverse of the torus of
circumstellar material: the difference is whether to place the
``blobs'' that provide the needed brightness contrast on the surface
of the star, or at the co-rotation radius.  In the spot occultation
model, the observing geometry would require an inner disk to be
projected into a narrow band across the stellar surface, in order to
produce variations with sufficiently short timescales.  The outer
disk would need either to not be present, or to be misaligned enough,
and narrow enough, that it did not fall along the line of sight. These
combined requirements seem geometrically contrived.
}

\replaced{The evolution of \ptfo\ over}{Observations of \ptfo\ from} the past decade
could offer additional hints.
\deleted{Specifically, }\ptfo's transition between having none, one, and
multiple dips per cycle seems important \added{(Figure~\ref{fig:o_minus_c})}.  It strains the ``enshrouded
protoplanet'' interpretation, because there are no known processes
that cause a planet's orbital phase to jump.  \deleted{The dips would then need
...
\added{
The H$\alpha$
excess observed by \citet{johnskrull_h_2016} also seems like an
important piece of evidence.
The model with circumstellar optically thick hydrogen
gas near the co-rotation radius readily explains this excess.
}

\added{Out of the available explanations, we therefore consider the
idea of a clumpy torus of optically thick dust or gas near the co-rotation radius
to be most plausible.
There is theoretical support for
the idea that accretion disks subject to strong magnetic
fields could enter stalled states and become trapped near co-rotation
\citep{dangelo_accretion_2012}.
The concept of an ``evaporating dust wall'' within which all
dust evaporates \citep{millan-gabet_circumstellar_2007,dullemond_inner_2010}
will likely be an important first-order consideration in
physical models of this scenario.
The additional question of whether the opacity in fact comes from dust 
or partially-ionized gas ({\it e.g.}, via gas continuum, atomic lines, or perhaps
TiO molecular opacity) remains a topic for future exploration.
}
...
dips is unresolved. Our preferred explanation is that a clumpy torus of dust or gas is near the Keplerian co-rotation radius, and periodically occults the star.
The \added{chromaticity, out-of-eclipse variability, and} jumping orbital phase \replaced{disfavored}{of \ptfo\ all disfavor} the explanation of an enshrouded,
transiting protoplanet.  Though \ptfo\ may not be a planet,\deleted{ as we and
others had hoped,} understanding it and its analogs is a worthy
problem\replaced{.}\added{that seems likely to provide insight into how gas
and dust disperse from young protostellar disks.} \replaced{It might even}{Understanding
this process would, in turn,} teach us about the birth environments of the
