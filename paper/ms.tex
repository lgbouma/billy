...
in fact already noted by \citet{stauffer_orbiting_2017}, the
asymmetric out-of-dip modulation shown by the TESS data puts the
connection on stronger footing.}
...
or {\it (vi)} dust clouds of \replaced{plausible
...
The explanations that \replaced{are not yet}{have not yet been} ruled
out include {\it (i)} transiting clumps of \added{dust or }gas at the
Keplerian co-rotation radius; {\it (ii)} transits of enshrouded
protoplanets; {\it (iii)} occultations of \replaced{starspots}{hot and
cold spots on the stellar surface} by an optically thick disk.  The
first and last explanations have added appeal because they are
...
\replaced{The evolution of \ptfo\ over}{Observations of \ptfo\ from}
the past decade could offer additional hints.  \deleted{Specifically,
}\ptfo's transition between having none, one, and multiple dips per
cycle seems important \added{(Figure~\ref{fig:o_minus_c})}.
\deleted{The dips would then need to be caused by material that was
somehow disrupted from the planet, but somehow remained co-orbital for
an extended duration. This seems implausible.} \added{ The H$\alpha$
excess observed by \citet{johnskrull_h_2016} also seems like an
important piece of evidence.  The model with circumstellar optically
thick hydrogen gas near the co-rotation radius readily explains this
excess.  }

\added{Out of the available explanations, we therefore consider the
idea of a clumpy torus of optically thick dust or gas near the
co-rotation radius to be most plausible.  There is theoretical support
for the idea that accretion disks subject to strong magnetic fields
could enter stalled states and become trapped near co-rotation
\citep{dangelo_accretion_2012}.  The concept of an ``evaporating dust
rim'' within which all dust evaporates
\citep{millan-gabet_circumstellar_2007,dullemond_inner_2010} will
likely also be an important first-order consideration in physical models of
this scenario.  The additional question of whether the opacity in fact
comes from dust or partially-ionized gas ({\it e.g.}, via gas
continuum, atomic lines, or perhaps TiO molecular opacity) also
