...
from billy.models import sinusoid_model, transit_model
...
y_mod = (
    sinusoid_model(sin_params, x_obs) +
...
if not os.path.exists(pklpath):
    with pm.Model() as model:
        ...
        t0 = pm.Uniform(
            "t0", lower=0, upper=true_d['period'], testval=true_d['t0']
        )
        logP = pm.Normal(
            "logP", mu=np.log(true_d['period']), sd=0.1,
            testval=np.log(true_d['period'])
        )
        period = pm.Deterministic("period", pm.math.exp(logP))
        u = xo.distributions.QuadLimbDark("u", testval=true_d['u'])
        r = pm.Uniform(
            "r", lower=0.02, upper=0.06,  testval=true_d['r']
        )
        b = xo.distributions.ImpactParameter(
            "b", ror=r, testval=true_d['b']
        )

        # set up Keplerian orbit for the planet, and compute model light-curve
        # via starry. track the value of the model light curve for plotting
        # purposes.
        orbit = xo.orbits.KeplerianOrbit(
            period=period, t0=t0, b=b, mstar=mstar, rstar=rstar
        )
        ...
        mu_model = (
            light_curve.flatten()
            +
            sinusoid_model([A0, omega0, phi0], x_obs)
        )

        likelihood = pm.Normal('obs', mu=mu_model,
