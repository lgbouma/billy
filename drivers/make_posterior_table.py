...
def main(modelid):
    ...
    summarypath = os.path.join(PLOTDIR, 'posterior_table_raw.csv')
    ...
    pr = {
        ...
        'r': normal_str(mu=prior_d['r'], sd=0.03*prior_d['r'], fmtstr='({:.4f}; {:.4f})'),
    ...
    outpath = os.path.join(PLOTDIR, 'posterior_table_clean.csv')
    df.to_csv(outpath, float_format='%.12f')
    print('made {}'.format(outpath))

    # df.to_latex is dumb with float formatting.
    outpath = os.path.join(PLOTDIR, 'posterior_table_clean.tex')
